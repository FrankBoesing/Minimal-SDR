...
#define ssd1306_swap(a, b) { typeof(a)t = a; a = b; b = t; }
...
void Adafruit_SSD1306::display(void) {
	...
	// I2C
	unsigned i = 0;
	int size = SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8;
	do {
		Wire.finish();
		Wire.beginTransmission(_i2caddr);
		Wire.write(0x40);
		unsigned chunk = min(256, size);
...
void Adafruit_SSD1306::display(int toline) { //FB
	Wire.finish();
	ssd1306_command(SSD1306_COLUMNADDR);
	ssd1306_command(0);   // Column start address (0 = reset)
	ssd1306_command(SSD1306_LCDWIDTH - 1); // Column end address (127 = reset)
	...
	// I2C
	unsigned i = 0;
	int size = toline * SSD1306_LCDWIDTH / 8;
	do {
		Wire.finish();
		Wire.beginTransmission(_i2caddr);
		Wire.write(0x40);
	...
	do {
		...
		do {
			...
			if (bits & (1 << n)) {
				...
				switch (textcolor)
				{
				case WHITE:   buffer[x1 + (y / 8) * SSD1306_LCDWIDTH] |=  (1 << (y & 7)); break;
				case BLACK:   buffer[x1 + (y / 8) * SSD1306_LCDWIDTH] &= ~(1 << (y & 7)); break;
				case INVERSE: buffer[x1 + (y / 8) * SSD1306_LCDWIDTH] ^=  (1 << (y & 7)); break;
				}
...
void Adafruit_SSD1306::drawFastHLine(int x, int y, int w, uint16_t color) {
	boolean bSwap = false;
	switch (rotation) {
	case 0:
...
void Adafruit_SSD1306::drawFastHLineInternal(int x, int y, int w, uint16_t color) {
...
void Adafruit_SSD1306::drawFastVLine(int x, int y, int h, uint16_t color) {
	bool bSwap = false;
	switch (rotation) {
	case 0:
		break;
...
void Adafruit_SSD1306::drawFastVLineInternal(int x, int __y, int __h, uint16_t color) {
